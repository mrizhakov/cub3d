...
/*   Updated: 2024/05/07 17:20:43 by ddavlety         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int fill(t_game *game_data, t_point size, t_point cur, int to_fill)
{
	...
	if (cur.y < 0 || cur.y >= size.y || cur.x < 0 || cur.x >= size.x)
		return (1);
	if (cur.y < 0 || cur.y >= size.y || cur.x < 0 || cur.x >= size.x || game_data->maze.g[cur.y][cur.x] != to_fill)
	{
		if (game_data->maze.g[cur.y][cur.x] == '0' || game_data->maze.g[cur.y][cur.x] == 'N' || game_data->maze.g[cur.y][cur.x] == 'E' || game_data->maze.g[cur.y][cur.x] == 'W' || game_data->maze.g[cur.y][cur.x] == 'S')
		{
...
int maze_check_closed(t_game *game_data)
{
	...
	is_closed = 0;
	maze_end.x = MAZE_DIMENSION; // is it the length of longest line?
