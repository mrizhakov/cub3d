...
/*   Updated: 2024/06/01 15:46:50 by ddavlety         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (ray.distance > ray_door.distance
		...
		slope = init_slope_data((uint32_t)game_data->player->x
				...
				* MINIMAP_SQUARE_SIDE_LEN / game_data->texture_width,
				...
				/ game_data->texture_width, game_data->img);
	else
		slope = init_slope_data((uint32_t)game_data->player->x
				* MINIMAP_SQUARE_SIDE_LEN / game_data->texture_width,
				(uint32_t)game_data->player->y * MINIMAP_SQUARE_SIDE_LEN
				...
				/ game_data->texture_width, game_data->img);
	draw_line(slope, game_data->player->color);
}
...
void	draw_fov(t_game *game_data)
{
	...
	while (column_id < game_data->num_rays)
	{
		...
		ray_angle = game_data->player_angle
			+ atan((column_id - game_data->num_rays / 2)
				/ game_data->dist_proj_plane);
