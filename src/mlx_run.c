...
/*   Updated: 2024/06/03 21:29:55 by ddavlety         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	game_data->mlx = mlx_init(WINDOW_WIDTH, WINDOW_HEIGHT, "Cub3D", true);
	if (!(game_data->mlx))
	{
		puts(mlx_strerror(mlx_errno));
		return (EXIT_FAILURE);
	}
	game_data->img = mlx_new_image(game_data->mlx, WINDOW_WIDTH, WINDOW_HEIGHT);
	if (!(game_data->img))
	{
